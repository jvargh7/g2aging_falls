...
mi_dfs <- readRDS(paste0(path_g2a_falls_folder,"/working/g2afelsa_sensitivity multiple imputation.RDS"))
...
for(i in 1:mi_dfs$m){
  ...
  gc();rm(df);rm(df_restricted);rm(ipcw_mod)
}
...
bind_rows(
  ...
  write_csv(.,"elsa/g2afe06_coefficients for sensitivity poisson regression.csv")
...
bind_rows(
  ...
  write_csv(.,"elsa/g2afe06_contrasts for sensitivity poisson regression with multiple imputation.csv")
