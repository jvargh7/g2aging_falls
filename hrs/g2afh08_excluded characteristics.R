...
excluded_df = hrs_all_65plus_control_available %>% 
  dplyr::filter(is.na(fu_fall_any))


hrs_excluded_svy <- excluded_df %>% 
...
hrs_sy <- svysummary(hrs_excluded_svy,
...
hrs_sy_total <- svysummary(hrs_excluded_svy,
                           c_vars = continuous_vars,
                           p_vars = proportion_vars,
                           g_vars = grouped_vars
) %>%
...
bind_rows(hrs_sy_total %>% mutate(gender = "Total"),
          hrs_sy) %>% 
  write_csv(.,path = "hrs/g2afh08_excluded characteristics.csv")
