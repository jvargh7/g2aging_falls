...
ind_covariates <- c(" + age + female + laborforce + iadl_some4 + fall_any + moderate_pa + vigorous_pa + polypharmacy + rxdiabi +
  balance_impairment + vision_impairment + heavydrinker + bmi + htn + raceeth + smoke")

hh_covariates <- c("+ hh_size + hh_wealthquintile + wave")

auxiliary_covariates <- c("+ adl_some6 + a1c_adj + sbp + dbp + tc_adj + hdl_adj + nfalls + agediagnosed_dm + 
